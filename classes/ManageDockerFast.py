...
class Docker:
    ...
    def run(self, model, language, task, output_format, output_dir, file_path, batch_size):
        """
        ...
        command = f'bash -c "source /home/rocm-user/.bashrc && cd /home/rocm-user/whisper && conda activate pytorch && python3 insanely-fast-whisper.py --model {model} --language {language} --output_format {output_format} --task {task} --output_dir {output_dir} --batch_size {batch_size} {file_path}"'
        print(f"Esecuzione del comando whisper...")
